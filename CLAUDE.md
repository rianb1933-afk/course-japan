...
`Teacher-Dashboard.html` memakai endpoint yang sama lewat aksi `class-overview` (anggota + progres + tugas) dan `create-assignment`. Progres siswa **sengaja diambil lewat server**, bukan langsung dari klien: ada policy warisan `"Leaderboard read"` yang mengizinkan setiap pengguna terautentikasi membaca SELURUH `user_progress` — termasuk `email`, `is_premium`, dan `plan` milik semua orang — padahal papan peringkat hanya memakai `name,xp,streak,level`. Policy itu terlalu longgar dan sebaiknya dipersempit; dashboard sengaja tidak bergantung padanya supaya tidak ikut rusak saat dipersempit. Kolom `email` tidak pernah dikembalikan ke klien.
