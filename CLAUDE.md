...
`Teacher-Dashboard.html` memakai endpoint yang sama lewat aksi `class-overview` (anggota + progres + tugas) dan `create-assignment`. Progres siswa **sengaja diambil lewat server**, bukan langsung dari klien. Dulu ada policy warisan `"Leaderboard read"` yang mengizinkan setiap pengguna terautentikasi membaca SELURUH `user_progress` — termasuk `email`, `is_premium`, dan `plan` milik semua orang — padahal papan peringkat hanya memakai `name,xp,streak,level`. RLS membatasi baris, bukan kolom, jadi begitu satu baris lolos seluruh kolomnya ikut terbaca. Policy itu **sudah dibuang**: papan peringkat sekarang memakai view `leaderboard` (`security_invoker = false`, `anon` dicabut) yang hanya memuat empat kolom itu, dan `getLeaderboard()` di `supabase-client.js` membacanya dari sana. Dashboard yang sejak awal tidak bersandar pada policy longgar itu tidak perlu diubah sama sekali. Kolom `email` tidak pernah dikembalikan ke klien.

Satu jebakan waktu yang sudah menggigit sekali: `supabase-client.js` memanggil `Auth.init()` di dalam listener `DOMContentLoaded`, sedangkan script inline halaman berjalan **sebelum** itu. Memeriksa sesi tanpa menunggu selalu menemukan `null`, dan halaman diam-diam jatuh ke mode demo meski pengajarnya sudah masuk — tanpa error apa pun. Tunda inisialisasi sampai dokumen siap.
