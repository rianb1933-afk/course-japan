...
function makeEvent({ method = 'GET', origin = 'https://nihonggopro.id', body = {}, query = {}, headers = {} } = {}) {
  return {
    httpMethod: method,
...
test('POST dengan tag yang diizinkan (p, h2, ul, li, strong, a) -> diterima', async () => {
  const { handler, insertCalls } = loadHandler({ roleRowResult: { role: 'admin' } });
  const html = '<h2>Judul</h2><p>Teks <strong>tebal</strong> dan <a href="https://nihonggopro.id">link</a>.</p><ul><li>Poin 1</li></ul>';
  const res = await handler(makeEvent({
    method: 'POST',
    headers: { authorization: 'Bearer faketoken' },
