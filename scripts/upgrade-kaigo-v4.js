...
function convert(f) {
  ...
  // === EXTRACT QUIZ DATA (PRESERVE EXACTLY) ===
  let quizData = '', qv = 'Q', quizScriptBlock = '';
  
  // Try to find the exact script block containing quiz data
  // Pattern 1: var QUIZ=[...];
  let qm = h.match(/(<script>\s*\nvar\s+QUIZ\s*=\s*\[[\s\S]*?\]\s*;?\s*\n<\/script>)/);
  if (qm) { quizData = qm[1]; qv = 'QUIZ'; }
  else {
    // Pattern 2: var QUIZ=[...] without newlines
    qm = h.match(/(<script>\s*var\s+QUIZ\s*=\s*\[[\s\S]*?\]\s*;?\s*<\/script>)/);
    if (qm) { quizData = qm[1]; qv = 'QUIZ'; }
  }
  if (!quizData) {
    // Pattern 3: var Q=[...],qi=...
    qm = h.match(/(<script>\s*\nvar\s+Q\s*=\s*\[[\s\S]*?\],\s*qi[\s\S]*?<\/script>)/);
    if (!qm) qm = h.match(/(<script>\s*var\s+Q\s*=\s*\[[\s\S]*?\],\s*qi[\s\S]*?<\/script>)/);
    if (qm) { quizData = qm[1]; qv = 'Q'; }
  }
  
  const quizCount = quizData ? ((quizData.match(/"q"/g)||[]).length || (quizData.match(/q:/g)||[]).length) : 0;
  ...
  for (const m of [...h.matchAll(/<h3[^>]*>([\s\S]*?)<\/h3>/g)]) {
    let t = m[1].replace(/<[^>]+>/g,'').trim().replace(/^[\p{Emoji_Presentation}\p{Emoji}\u200d\uFE0F]+\s*/u, '').trim();
  ...
  // === CONTENT EXTRACTION ===
  // Strategy: extract everything between <body> and first <script> tag
  const bodyIdx = h.indexOf('<body>');
  if (bodyIdx === -1) return 'error: no <body>';
  
  // Find the first <script> tag after body
  let firstScriptIdx = h.indexOf('<script>', bodyIdx + 6);
  if (firstScriptIdx === -1) firstScriptIdx = h.length;
  
  let content = h.substring(bodyIdx + 6, firstScriptIdx);
  
  // Remove old skip-to-content link
  content = content.replace(/<a href="#main-content"[^>]*>[\s\S]*?<\/a>/, '');
  
  // Remove old km-illustration (nested divs)
  ...
  // Remove old <main id="main-content"> tag if present
  content = content.replace(/<main id="main-content">/g, '');
  ...
  // Remove old vocab grid
  content = content.replace(/<div class="pg-vocab" id="pg"><\/div>/g, '');
  
  // Remove old dialog headers (keep content)
  content = content.replace(/<div><div>(🧠|💊|💬|⚖️)\s*Dialog \d+[^<]*<\/div><\/div>/g, '<h3>$1</h3>');
  ...
  // Remove closing </div></div> that matched wrap
  // Be careful not to remove too many
  ...
  // Remove </main> and </body> that came from extraction
  content = content.replace(/<\/main>[\s\S]*$/g, '');
  content = content.replace(/<\/body>[\s\S]*$/g, '');
  
  content = content.trim();
  
  // Remove trailing </div> that don't have matching opens (cleanup)
  ...
  if (closes > opens) {
    ...
    for (let i = 0; i < excess; i++) {
      ...
      if (lastClose > -1) content = content.substring(0, lastClose) + content.substring(lastClose + 6);
    }
  }
