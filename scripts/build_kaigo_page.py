...
/* Baris chip ini menggulir mendatar (isinya 1.445px pada wadah 1.145px) tapi
   scrollbar-nya disembunyikan — enam kategori terakhir bisa dijangkau namun
   tidak ada tanda apa pun bahwa ia bisa digulir. Diukur di Chromium: chip
   "Hukum & Etika" berakhir di 1.512px, jauh di luar wadah.

   DUA PERCOBAAN YANG GAGAL, DAN KENAPA

   1. bayangan gulir CSS murni (background berlapis, attachment local/scroll)
      Chip-nya buram dan menutupi latar wadahnya, jadi bayangannya tidak
      pernah terlihat.
   2. scrollbar tipis yang terlihat (scrollbar-width + scrollbar-color)
      Lebih baik daripada disembunyikan, tapi macOS memakai overlay scrollbar
      yang hanya muncul saat menggulir — jadi TIDAK BISA DIVERIFIKASI di sini.

   Yang dipakai: mask-image pada wadahnya. Ia memudarkan CHIP-nya sendiri,
   bukan latar di belakangnya, jadi pasti terlihat apa pun yang menimpanya —
   dan bisa dibuktikan dengan tangkapan layar.

   Pudarnya hanya di sisi kanan. Di sisi kiri pudar akan memotong chip
   "Semua" yang aktif pada keadaan awal, padahal di sana tidak ada isi
   tersembunyi. */
