...
// interaktif mendalam bahwa KEDUA panggilan `renderQuiz()` existing
// (di bawah, dalam event listener "quiz-next" dan "btn-retry")
// genuinely HANYA terpicu SETELAH pengguna berinteraksi -- namun
// genuinely TIDAK ADA cara memicu interaksi tersebut karena
// `#quiz-options` genuinely SELALU KOSONG saat load (render pertama
// tidak pernah terjadi) dan tombol "Soal Berikutnya" genuinely baru
// `visible` SETELAH menjawab (`handleAnswer()`, baris di atas) --
// menciptakan DEAD-END SEMPURNA: kuis genuinely tidak bisa dipakai
// sama sekali oleh siapapun. Dibuktikan konkret via pengujian
// Playwright: `#quiz-options` genuinely `childrenCount: 0` bahkan
// setelah menunggu lama, meski data `questions` genuinely valid.
renderQuiz();
...
// ── SCROLL REVEAL — Semua elemen langsung visible (no-hide approach) ──
(function () {
  // Tambah .visible ke semua .reveal segera — CSS sudah handle visibility
  // .js class tidak perlu karena CSS fallback opacity:1 di semua kondisi
  var revEls = document.querySelectorAll('.reveal');
  revEls.forEach(function (el) { el.classList.add('visible'); });
  // Optional: subtle entrance animation via IntersectionObserver jika tersedia
...
// Elemen navbar LAMA; navbar kini di-inject kyoto-navbar.min.js dengan ID berbeda.
// Tanpa guard, fungsi ini melempar TypeError saat login/logout/muat sesi dan
// menghentikan seluruh skrip halaman depan.
