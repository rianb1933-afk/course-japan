(function () {
  ...
  document.addEventListener('keydown', (event) => {
    ...
    if (event.key === 'Escape') {
      const openPanelSelectors = [
        ...
        '.nihongo-reco-panel', '.nihongo-search-panel', '.nihonggo-pro-panel',
      ];
  ...
  if (!document.querySelector('.nihongo-a11y-panel')) {
    ...
    panel.addEventListener('click', (event) => {
      ...
      if (action === 'summary') panel.querySelector('.nihongo-a11y-output').textContent = `Ringkasan: ${document.querySelector('h1')?.textContent || document.title}. Halaman ini berisi materi/fitur utama Nihonggo Pro Academy dan navigasi belajar JLPT.`;
  ...
  if (!document.querySelector('.nihongo-collab-panel')) {
    ...
    document.body.appendChild(panel);
    const channel = 'BroadcastChannel' in window ? new BroadcastChannel('nihonggo-pro-collab') : null;
    const render = () => {
  ...
  const markPageComplete = () => {
    ...
    document.querySelector('.nihonggo-pro-panel')?._renderPro?.();
  };
  ...
  if (!document.querySelector('.nihonggo-pro-panel')) {
    const panel = document.createElement('section');
    panel.className = 'nihonggo-pro-panel';
    panel.innerHTML = `
      ...
      <div class="nihongo-panel-body">
        <div class="nihonggo-pro-tabs" role="tablist">
        ...
        <div class="nihonggo-pro-view" data-pro-view="overview"></div>
        <div class="nihonggo-pro-view" data-pro-view="goal" hidden></div>
        <div class="nihonggo-pro-view" data-pro-view="notes" hidden></div>
      </div>`;
    document.body.appendChild(panel);
    ...
    const renderPro = () => {
      ...
      overview.innerHTML = `
        <div class="nihonggo-pro-metrics">
        ...
        <div class="nihonggo-pro-goal-card">
        ...
        <div class="nihonggo-pro-actions">
      ...
      panel.querySelector('[data-pro-view="notes"]').innerHTML = `
        <div class="nihonggo-pro-note-head"><b>${htmlEscape(pageLabel)}</b><span>Catatan tersimpan per halaman</span></div>
        <textarea class="nihonggo-pro-note" data-pro-note placeholder="Ringkas poin penting, grammar yang membingungkan, atau contoh kalimat pribadi...">${htmlEscape(note)}</textarea>
        <button class="nihongo-panel-action" type="button" data-pro-save-note>Simpan Catatan</button>
        <div class="nihonggo-pro-list">${Object.entries(data.notes || {}).slice(-5).reverse().map(([id, text]) => `<div><b>${htmlEscape(id.split('/').pop() || 'Catatan')}</b><span>${htmlEscape(text).slice(0, 110)}</span></div>`).join('') || '<div><b>Belum ada catatan</b><span>Catatan cepat akan muncul di sini.</span></div>'}</div>`;
    };
    ...
    panel.addEventListener('click', (event) => {
      ...
      if (event.target.closest('[data-pro-export]')) {
        ...
        a.href = url;
        a.download = `nihonggo-progress-${new Date().toISOString().slice(0, 10)}.json`;
        a.click();
  ...
  window.openProfessionalHub = () => {
    const panel = document.querySelector('.nihonggo-pro-panel');
    panel?._renderPro?.();
    panel?.classList.toggle('open');
  };
  ...
  const canonicalUrl = `${location.origin || 'https://nihonggopro.id'}${location.pathname}`;
  ...
  if (!document.querySelector('script[type="application/ld+json"][data-nihongo-schema]')) {
    ...
    schema.textContent = JSON.stringify({
      ...
      image: ogImage,
      inLanguage: ['id', 'ja'],
      provider: { '@type': 'Organization', name: 'Nihonggo Pro Academy' },
  ...
  const requestReminderPermission = async () => {
    ...
    if (permission === 'granted') {
      ...
      new Notification('Nihonggo Pro Academy', { body: 'Reminder harian aktif. Jaga streak belajarmu hari ini.' });
    }
  };
  ...
  document.querySelectorAll('img:not([alt])').forEach((img) => {
    img.alt = img.closest('figure')?.querySelector('figcaption')?.textContent?.trim() || 'Ilustrasi Nihonggo Pro Academy';
  });
